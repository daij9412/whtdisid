...
require(knitr); require(lavaan); require(xtable); require(emmeans); require(mctest)

#load in data (study1a_clean, study1b_clean, study2_clean, studyPLT_clean)
...
study1b_clean$condition <- as.factor(study1b$condition)
...
##manipulation check 
MPcheck_1b <- t.test(TrumpProto ~ condition, data = study1b_relevel)
summary(MPcheck_1b)
...
levels(study2_clean$condition) <- c("Control", "Actual threat", "High threat")
...
summary(MPcheck_2)
TukeyHSD(MPcheck_2, "condition_control", ordered = TRUE)
